...
# Addding Certificates for RabbitMQ for SSL.
...
cookbook_file '/etc/certs/rabbitmq_certs.tgz' do
  source 'rabbitmq_certs.tgz'
  mode '755'
end

execute 'extracting_ssl_certs' do
  command 'cd /etc/certs && tar -xvzf rabbitmq_certs.tgz && chown rabbitmq:rabbitmq *'
end

# Adding Template for definition loading.
cookbook_file "#{node['rabbitmq']['config_root']}/load_definitions.json" do
  source 'load_definitions.json'
  owner 'root'
  group 'root'
  mode '644'
